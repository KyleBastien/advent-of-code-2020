...
export const MY_BAG = 'shiny gold';

const leftBagRegex = /(\w+ \w+) bags contain/;
const containsBagRegexNoCount = /\d+ (\w+ \w+) bags?/g;
const containsBagRegexWithCount = /(\d+) (\w+ \w+) bags?/g;
// TODO: I should be able to reuse this logic better than I am
export const parseBagRules = () => {
  const rules: { [key: string]: string[] } = {};
  for (const rule of bagRules) {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    let match = leftBagRegex.exec(rule)!;
    const current = match[1];
    rules[current] = [];
    const children = rule.substring(match[0].length);
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    while ((match = containsBagRegexNoCount.exec(children)!)) {
...
export const parseBagRulesForCount = () => {
  const rules: { [key: string]: [string, number][] } = {};
  for (const rule of bagRules) {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    let match = leftBagRegex.exec(rule)!;
    const current = match[1];
    ...
    const children = rule.substring(match[0].length);
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    while ((match = containsBagRegexWithCount.exec(children)!)) {
