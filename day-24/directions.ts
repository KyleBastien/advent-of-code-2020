...
export const getCounts = () => {
  const counts = new Map<string, [Point2D, number]>();
  ...
  for (const line of directions) {
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      const h = HexDirection[s.toUpperCase()];
      x += h[0];
      y += h[1];
    }

    const p = new Point2D(x, y);
    const existingValue = counts.get(p.toString());
    if (existingValue) {
      counts.set(p.toString(), [existingValue[0], existingValue[1] + 1]);
    } else {
      counts.set(p.toString(), [p, 1]);
    }
  }
  return counts;
};
