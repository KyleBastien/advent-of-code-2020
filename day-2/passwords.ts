...
export interface ParsedPassword {
  minimumNumberOfLetters: number;
  maximumNumberOfLetters: number;
  letterToFind: string;
  password: string;
}

export const parsePassword = (password: string): ParsedPassword => {
  const pieces = password.split(' ');
  const letterToFind = pieces[1].substring(0, 1);
  const numbers = pieces[0].split('-');
  const minimumNumber = parseInt(numbers[0], 10);
  const maximumNumber = parseInt(numbers[1], 10);
  return {
    minimumNumberOfLetters: minimumNumber,
    maximumNumberOfLetters: maximumNumber,
    letterToFind,
    password: pieces[2],
  };
};
