...
export enum PassportFieldNames {
  byr = 'byr',
  iyr = 'iyr',
  eyr = 'eyr',
  hgt = 'hgt',
...
export const validPassportFields = [
  PassportFieldNames.byr,
  PassportFieldNames.iyr,
  PassportFieldNames.eyr,
  PassportFieldNames.hgt,
  PassportFieldNames.hcl,
  PassportFieldNames.ecl,
  PassportFieldNames.pid,
];

interface ParsedPassport {
  passportBrokenUpByField: string[];
  fullPassportString: string;
}

export const parsePassports = (passports: string[]): ParsedPassport[] => {
  const parsedPassports: ParsedPassport[] = [];
  let passportString = '';
  for (const passport of passports) {
    if (passport !== '') {
      passportString += passport + ' ';
    } else {
      parsedPassports.push(splitUpPassword(passportString));
      passportString = '';
    }
  }
  if (passportString) {
...
const splitUpPassword = (passportString: string): ParsedPassport => {
  const passportBrokenUpByField = passportString.split(' ');
  return {
    passportBrokenUpByField,
    fullPassportString: passportString,
  };
};
